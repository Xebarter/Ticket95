...
export default function SupportMessagesAdminPage() {
  const { data, error, isLoading } = useSWR('support-messages', fetchSupportMessages);
            ...
            <TableBody>
              {messages.map((m: any) => (
                <TableRow key={m.id} className="align-top">
                  ...
                  <TableCell>
                    <Badge variant={m.status === 'resolved' ? 'default' : 'secondary'} className="rounded-full text-[10px]">
                      {m.status}
                    </Badge>
                  </TableCell>
